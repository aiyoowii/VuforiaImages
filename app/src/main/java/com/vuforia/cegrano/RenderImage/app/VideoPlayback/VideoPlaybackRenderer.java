...
//            1.0f, 0.0f, 1.0f,};
    private float videoQuadTextureCoords[] = {0.0f, 1.0f, 1.0f, 1.0f, 1.0f,
            0.0f, 0.0f, 0.0f,};
...
//        for (int tIdx = 0; tIdx < state.getNumTrackableResults(); tIdx++) {
        ...
        if (state.getNumTrackableResults() == 0) {
            ...
            // calculation
            Log.e("image target", imageTarget.getName());
            for (int m = 0; m < mActivity.mMovieName.length; m++) {
                if (mActivity.mMovieName[m].contains(imageTarget.getName())) {
                    currentTarget = m;
                    break;
                }
            }
...
//                currentTarget = 0;
            ...
            if ((mVideoPlayerHelper[currentTarget].getStatus() == MEDIA_STATE.PAUSED)
                ...
                if ((mVideoPlayerHelper[currentTarget].getStatus() == MEDIA_STATE.REACHED_END))
                    mSeekPosition[currentTarget] = 0;

                mVideoPlayerHelper[currentTarget].play(false,
                        mSeekPosition[currentTarget]);
                mSeekPosition[currentTarget] = VideoPlayerHelper.CURRENT_POSITION;
            } else if (mVideoPlayerHelper[currentTarget].getStatus() == MEDIA_STATE.PLAYING) {
                // If it is playing then we pause it
            }

            // If the movie is ready to start playing or it has reached the end
            // of playback we render the keyframe
            Log.e("Status", mVideoPlayerHelper[currentTarget].getStatus() + "");
            Log.e("Status", currentStatus[currentTarget] + "");
...
//                        modelViewMatrixVideo, 0);
                ...
                Matrix.scaleM(modelViewMatrixVideo, 0,
                        targetPositiveDimensions[currentTarget].getData()[0],
                        targetPositiveDimensions[currentTarget].getData()[1],
...
//                            fillBuffer(videoQuadTextureCoordsTransformedChips));
                GLES20.glVertexAttribPointer(videoPlaybackTexCoordHandle,
                        2, GLES20.GL_FLOAT, false, 0,
                        fillBuffer(videoQuadTextureCoords));
